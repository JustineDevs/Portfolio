          ...
          {projects.map((project) => (
            ...
            >
              ...
              <div className="absolute inset-0 z-10 p-3 xs:p-4 sm:p-5 flex flex-col justify-between pointer-events-none">
                <div>
                  <h3 className="text-white text-[14px] xs:text-[16px] sm:text-[18px] font-bold mb-1">{project.title.toUpperCase()}</h3>
                  {project.description && (
                    <p className="text-white/80 text-[10px] xs:text-[11px] sm:text-[12px] leading-tight mb-1 xs:mb-2 line-clamp-2">{project.description}</p>
                  )}
                  {project.websiteUrl && (
