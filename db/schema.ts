...
export const providerUsageSnapshots = sqliteTable(
  "provider_usage_snapshots",
  {
    ...
    tokenCountKind: text("token_count_kind").notNull().default("exact"),
    cachedTokens: integer("cached_tokens").notNull().default(0),
    activityCount: integer("activity_count").notNull().default(0),
    modelBreakdown: text("model_breakdown"),
    estimatedCost: real("estimated_cost").notNull().default(0),
    costCurrency: text("cost_currency").notNull().default("USD"),
    originalCost: real("original_cost"),
    originalCurrency: text("original_currency"),
    exchangeRateToUsd: real("exchange_rate_to_usd"),
    sourceHash: text("source_hash"),
