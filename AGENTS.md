...
- GitHub Actions: CI workflow runs on push and pull request to `main` or `master` (pnpm 9, Node 20, lint, test, build with a non-secret placeholder `AUTH_SECRET`); a separate workflow publishes a GitHub Release with auto-generated notes when a `v*` tag is pushed.
